...
const IN = path.resolve(`data/sources/${DATE}/property.json`);
const OUT = path.resolve('data/partials/property.json');
function readJSON(p){ try { return JSON.parse(fs.readFileSync(p,'utf8')); } catch { return null; } }
const src = readJSON(IN);
if (!src) { console.warn(`[property] No source at ${IN}`); process.exit(0); }
